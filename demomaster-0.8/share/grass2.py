# clcheung, Mar 2009
# support grass model
# grass model has one shader, one texture, and a list of leaf model
...
class GrassModel():
    def __init__(self, name, shaderfile, texturefile,jitter, leafmodels):
        self.texturefile = texturefile
        self.shaderfile = shaderfile
        self.name = name
        self.leafmodels = leafmodels
        self.jitter = jitter

class LeafModel():
    def __init__(self, name, nrplates, width, height, uvlist): #, jitter=-1):
        ...
        for i in range(nrplates):
            if uvlist != None:
                ...
                maker.setUvRange(Point2(0,0),Point2(1,0.95))
        ...
        # this jitter ratio is moved to grass model
        #if jitter == -1:
...
class GrassNode(demobase.Att_base):
    def __init__(self, name, parentnode=render):
...
##        return np
    ...
    # it accept a grass model instead of leaf model
    ...
    def addGrassWithGrassModel(self, pos, heading, leafindex):
        np = self.model.leafmodels[leafindex].np.copyTo( self.grassNP )
        np.setTwoSided( True )
        np.setHpr(Vec3(heading,0,0))
        np.setPos(pos)
        return np
