...
class World(DirectObject):
    def __init__(self):
        ...
        self.inst_l = addInstructions(0.80, 'Mouse wheel : lift up/down camera')
        ...
        floor.flattenStrong()
        wall = floor.copyTo(render)
        wall.setPos(wall,Vec3(0,20,20))
        wall.setP(90)
        wall1 = floor.copyTo(render)
        wall1.setPos(wall,Vec3(-20,0,20))
        wall1.setR(90)
        ...
        self.fr = loader.loadModel("smiley")
        self.fr.reparentTo(render)
        self.fr.setPos(0,-10,40)
        self.ivals = {}
        for x in xrange(3):
            tmp = NodePath("c"+str(x))
            tmp.reparentTo(render)
            tmp.setPos(0,-10,20)
            tmp1 = self.fr.copyTo(tmp)
            tmp1.setPos(randrange(5,15),randrange(5,15),randrange(5,15))
            p = Point3(0,0,0)
            p[x] = 360
            if x < 2: p[x+1] = 360
            else: p[0] = 360
            self.ivals[x] = tmp.hprInterval(10,p)
            self.ivals[x].loop()
        ...
        self.accept('Z',self.adjustPushBias,[0.9])
        self.accept('wheel_up', self.adjust_light_h,[1])
        self.accept('wheel_down', self.adjust_light_h,[-1])
    
        self.LCam=base.makeCamera(LBuffer)
        #self.LCam.node().setLens(OrthographicLens())
        self.LCam.node().setScene(render)
        self.LCam.node().getLens().setFov(75)
        self.LCam.node().getLens().setNearFar(0,100)
        print self.LCam.node().getLens().getConvergenceDistance(),\
                self.LCam.node().getLens().getFocalLength(),\
                self.LCam.node().getLens().getNear(),\
                self.LCam.node().getLens().getInterocularDistance ()
...
# end of __init__
    def adjust_light_h(self,value):
        self.LCam.setPos(self.LCam.getPos()+Vec3(0,0,value))
        self.fr.setPos(self.fr.getPos()+Vec3(0,0,value))
        print self.LCam.getPos()
